  ...
  export default {
    ...
    methods: {
      ...
      pageRequest() {
        ...
        axios.get('/api/report/crm/productMonthPriceSum', {params: submitData}).then((response) => {
      ...
      exportData(){
        ...
        util.confirmBeforeAction(this, "最多导出50000条记录，确认导出?").then(() => {
          window.location.href='/api/report/crm/productMonthPriceSum/export?'+qs.stringify(util.deleteExtra(this.formData));
        }).catch(()=>{});
      },
      uploadAudit(){
        if(!this.formData.areaId) {
        ...
        util.confirmBeforeAction(this,"确认审核通过吗?").then(()=>{
          axios.get('/api/report/crm/productMonthPriceSum/uploadAudit',{params: util.deleteExtra(this.formData)}).then((response)=>{
    ...
    created() {
      ...
      axios.get('/api/report/crm/productMonthPriceSum/getQuery').then((response) => {
