 ...
 */
interface  CustomerReceiveRepository{
    @Query("""
     SELECT
        temp.customerId,
    ...
    @Query("""
      SELECT
        ...
        AND t01.FCONTACTUNIT IN :#{#p.customerIdList}

        AND t01.FDATE >=:#{#p.dateStart}
        AND t01.FDATE <:#{#p.dateEnd}
        ...
        AND t11.FRETCUSTID IN ? :#{#p.customerIdList}
        AND t11.FDATE >=:#{#p.dateStart}
        AND t11.FDATE <:#{#p.dateEnd} GROUP BY t11.FID,
        t11.FRETCUSTID,
        t12.FNAME,
        t11.FBillNo,
        t11.FDATE,
        ...
        AND t21.FCONTACTUNIT IN :#{#p.customerIdList}
        ...
        AND t31.FCONTACTUNIT IN :#{#p.customerIdList}
        AND t31.FDATE >=:#{#p.dateStart}
        AND t31.FDATE <:#{#p.dateEnd}
        ...
        AND t41.FCUSTOMERID IN :#{#p.customerIdList}
        AND t41.FDATE >=:#{#p.dateStart}
        AND t41.FDATE <:#{#p.dateEnd} GROUP BY  t41.FID,
        t41.FCUSTOMERID,
        t42.FNAME,
        t41.FBillNo,
        t41.FDATE,
        ...
        AND t52.FNAME = '现销退货单'
        AND t51.FRETCUSTID IN :#{#p.customerIdList}
        AND t51.FDATE >=:#{#p.dateStart}
        AND t51.FDATE <: #{#p.dateEnd} GROUP BY t51.FID,
        t51.FRETCUSTID,
        t52.FNAME,
        t51.FBillNo,
        t51.FDATE,
        ...
        AND t62.FNAME = '现销出库单'
        AND t61.FCUSTOMERID IN :#{#p.customerIdList}
        AND t61.FDATE >=:#{#p.dateStart}
        AND t61.FDATE <:#{#p.dateEnd}  GROUP BY  t61.FID,
        t61.FCUSTOMERID,
        t62.FNAME,
        t61.FBillNo,
        t61.FDATE,
    ...
    @Query("""
       SELECT
        ...
        AND t11.FCUSTOMERID IN :#{#p.customerIdList}
        AND t11.FDATE >:#{#p.dateStart}
        AND t11.FDATE <:#{#p.dateEnd}
        ...
        AND t21.FRETCUSTID IN :#{#p.customerIdList}
        AND t21.FDATE >=:#{#p.dateStart}
        AND t21.FDATE <:#{#p.dateEnd}
     """, nativeQuery = true)
    ...
    @Query("""
     SELECT
        ...
        WHERE
        t1.FID = t2.FID
        AND t1.FCONTACTUNIT IN ?:#{#p.customerIdList}
        AND t1.FDATE  >:#{#p.dateStart}
        AND t1.FDATE <:#{#p.dateEnd}
        UNION ALL
        SELECT
        t3.FBILLNO AS name,
        ...
        WHERE
        t3.FID = t4.FID
        AND t3.FCONTACTUNIT  IN ?:#{#p.customerIdList}
        ...
        WHERE
        t5.FID = t6.FID
        AND t5.FCONTACTUNIT IN ?:#{#p.customerIdList}
