...
class BdAccountRepository @Autowired constructor(val namedParameterJdbcTemplate: NamedParameterJdbcTemplate) {
    ...
    fun findByFullName(fullName:String):BdAccount?{
        var list = namedParameterJdbcTemplate.query("""
            select
                t1.FACCTID,
                t2.FFULLNAME,
            ...
            where
                ...
                AND t1.FDOCUMENTSTATUS = 'C'
                AND t1.FFORBIDSTATUS = 'A'
                and t2.FFULLNAME = :fullName
        """,Collections.singletonMap("fullName",fullName),BeanPropertyRowMapper(BdAccount::class.java))
        if(list.size>0){
            return list[0]
        }else{
            return null
