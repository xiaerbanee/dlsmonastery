<template>
  <div>
    ...
    <div>
      ...
      <search-dialog @enter="search()" :show="formVisible" @hide="formVisible=false" :title="$t('menuCategoryList.filter')"  v-model="formVisible" size="tiny" class="search-form" z-index="1500" ref="searchDialog">
      ...
      <el-table :data="page.content" :height="pageHeight" style="margin-top:5px;" v-loading="pageLoading" :element-loading-text="$t('menuCategoryList.loading')" @sort-change="sortChange" stripe border>
        ...
        <el-table-column fixed="right" :label="$t('menuCategoryList.operation')" width="140">
          <template scope="scope">
              <el-button size="small" @click.native="itemAction(scope.row.id,'edit')" v-permit="'sys:menuCategory:edit'">{{$t('menuCategoryList.edit')}}</el-button>
              <el-button size="small" @click.native="itemAction(scope.row.id,'delete')" v-permit="'sys:menuCategory:delete'">{{$t('menuCategoryList.delete')}}</el-button>
          </template>
        </el-table-column>
      </el-table>
