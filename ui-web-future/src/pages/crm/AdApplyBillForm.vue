<template>
  <div>
    <head-tab :active="$t('adApplyBillForm.adApplyBillForm')"></head-tab>
    <div>
      <el-form :model="inputForm" ref="inputForm" :rules="rules" label-width="120px" class="form input-form">
        <el-form-item :label="$t('adApplyBillForm.billType')" prop="billType">
          <el-select v-model="inputForm.billType" :placeholder="$t('adApplyBillForm.selectInput')" :clearable=true @change="onchange">
            <el-option v-for="billType in formProperty.billTypes" :key="billType" :label="billType" :value="billType"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item :label="$t('adApplyBillForm.billDate')" prop="billDate">
          <el-date-picker  v-model="inputForm.billDate" type="date" align="left" :placeholder="$t('adApplyBillForm.selectDate')" format="yyyy-MM-dd" ></el-date-picker>
        </el-form-item>
        <el-form-item :label="$t('adApplyBillForm.expressCompany')"  prop="expressCompanyId">
          <el-select v-model="inputForm.expressCompanyId" :placeholder="$t('adApplyBillForm.selectInput')" :clearable=true @change="onchange">
            <el-option v-for="expressCompany in formProperty.expressComanys" :key="expressCompany.id" :label="expressCompany.name" :value="expressCompany.id"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item :label="$t('adApplyBillForm.billRemarks')"  prop="billRemarks">
          <el-input v-model="inputForm.billRemarks" type="textarea"></el-input>
        </el-form-item>
        <el-form-item :label="$t('adApplyBillForm.financeQty')" >
          {{store.name}}
        </el-form-item>
        <el-form-item>
          <el-button type="primary" :disabled="submitDisabled" @click="formSubmit()">{{$t('adApplyBillForm.save')}}</el-button>
        </el-form-item>
      </el-form>
      <el-row :gutter="20" style="margin-bottom:20px;float:right">
        <span>{{$t('adApplyBillForm.search')}}</span>
       <el-input v-model="productName" @change="searchDetail" :placeholder="$t('adApplyBillForm.inputTowKey')" style="width:200px;margin-right:10px"></el-input>
     </el-row>
      <el-table :data="filterAdApplyList"  stripe border>
        <el-table-column prop="shop.name" :label="$t('adApplyBillForm.shopName')" ></el-table-column>
        <el-table-column prop="product.code" :label="$t('adApplyBillForm.productCode')" ></el-table-column>
        <el-table-column prop="product.name" sortable :label="$t('adApplyBillForm.productName')" ></el-table-column>
        <el-table-column prop="createdDate" sortable :label="$t('adApplyBillForm.createdDate')"></el-table-column>
        <el-table-column prop="applyQty" :label="$t('adApplyBillForm.applyQty')" ></el-table-column>
        <el-table-column prop="confirmQty" sortable :label="$t('adApplyBillForm.confirmQty')" ></el-table-column>
        <el-table-column prop="leftQty" :label="$t('adApplyBillForm.leftQty')" ></el-table-column>
        <el-table-column prop="billQty" :label="$t('adApplyBillForm.billQty')" >
          <template scope="scope">
            <el-input v-model="scope.row.billQty"></el-input>
          </template>
        </el-table-column>
        <el-table-column prop="storeQty" sortable :label="$t('adApplyBillForm.financeQty')" ></el-table-column>
        <el-table-column prop="adApply.product.price2" :label="$t('adApplyBillForm.price')" ></el-table-column>
      </el-table>
    </div>
  </div>
</template>
<script>
  export default{
    data(){
      return{
        submitDisabled:false,
        productName:"",
        filterAdApplyList:[],
        formProperty:{},
        store:{name:''},
        applyQtys:"",
        leftQtys:"",
        confirmQtys:"",
        inputForm:{
          billType:'',
          billDate: util.currentDate(),
          expressCompanyId:'',
          billRemarks:'',
          billQtys:'',
          adApplyList:[],
        },
        rules: {
          billType: [{ required: true, message: this.$t('adApplyBillForm.prerequisiteMessage')}],
          billDate: [{ required: true, message: this.$t('adApplyBillForm.prerequisiteMessage')}],
          expressCompanyId: [{ required: true, message: this.$t('adApplyBillForm.prerequisiteMessage')}],
        },
        remoteLoading:false
      }
    },
    methods:{
      formSubmit(){
        this.submitDisabled = true;
        var form = this.$refs["inputForm"];
        form.validate((valid) => {
          if (valid) {
            axios.post('/api/crm/adApply/billSave',qs.stringify(this.inputForm,{allowDots:true})).then((response)=> {
              this.$message(response.data.message);
              if(this.isCreate){
                form.resetFields();
                this.submitDisabled = false;
              } else {
                this.$router.push({name:'adApplyList',query:util.getQuery("adApplyList")})
              }
            });
          }else{
            this.submitDisabled = false;
          }
        })
      },
      searchDetail(){
        var val=this.productName;
        var tempList=new Array();
        for(var index in this.inputForm.adApplyList){
          var detail=this.inputForm.adApplyList[index];
          if((util.contains(detail.product.name,val)||util.isBlank(val)) ){
            tempList.push(detail)
          }
        }
        this.filterAdApplyList = tempList;
      },onchange(){
        if(this.inputForm.billType != ''){
          this.submitDisabled = true;
          axios.get('api/crm/adApply/getBillAdApplyMap',{params:{billType:this.inputForm.billType}}).then((response) =>{
            for(var index in response.data.adApplyList){
              response.data.adApplyList[index].billQty=response.data.adApplyList[index].leftQty
            }
            this.inputForm.adApplyList = response.data.adApplyList;
            this.filterAdApplyList = response.data.adApplyList;
            this.store = response.data.store;
            this.applyQtys=response.data.applyQtys;
            this.leftQtys=response.data.leftQtys;
            this.confirmQtys=response.data.confirmQtys;
            this.submitDisabled = false;
          });
        }
      },getShopTypeLabel(id) {
        var arr = this.shops;
        if(id){
          for(var i in arr){
            if(arr[i].id === id){
              this.shopTypeLabel =  arr[i].typeLabel;
              return;
            }
          }
        }
      }
    },created () {
      this.pageHeight = window.outerHeight -320;
      axios.get('api/crm/adApply/getBillFormProperty').then((response) =>{
        this.formProperty = response.data;
        this.inputForm.expressCompanyId=response.data.expressCompanyId
      });
    }
  }
</script>
