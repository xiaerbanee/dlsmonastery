<template>
  <div>
    <head-tab :active="$t('expressCompanyList.expressCompanyList') "></head-tab>
    <div>
      <el-row>
  ...
  export default {
    ...
    methods: {
      pageRequest() {
        ...
        axios.get('/api/crm/expressCompany',{params:this.formData}).then((response) => {
        ...
        if(action=="修改") {
          ...
          axios.get('/api/crm/expressCompany/delete',{params:{id:id}}).then((response) =>{
      ...
      axios.get('/api/crm/expressCompany/getListProperty').then((response) =>{
