<template>
  <div>
    <head-tab :active="$t('depotForm.depotForm') "></head-tab>
    <div>
  ...
  export default{
    ...
    methods:{
      formSubmit(){
        ...
        form.validate((valid) => {
          if (valid) {
            axios.post('/api/crm/depot/save', qs.stringify(this.inputForm)).then((response)=> {
      ...
      if (query !== '') {
        ...
        if (query !== '') {
          this.remoteLoading = true;
          axios.get('/api/crm/depot/search',{params:{name:query,category:"STORE"}}).then((response)=>{
      ...
      axios.get('/api/crm/depot/getFormProperty').then((response)=>{
        this.formProperty=response.data;
      });
      if(!this.isCreate){
        axios.get('/api/crm/depot/findOne',{params: {id:this.$route.query.id}}).then((response)=>{
