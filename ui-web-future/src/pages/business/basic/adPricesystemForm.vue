<template>
  <div>
    <head-tab :active="$t('adPricesystemForm.adPricesystemForm') "></head-tab>
    <div >
...
<script>
    export default{
      ...
      methods:{
        formSubmit(){
          ...
          form.validate((valid) => {
            if (valid) {
              ...
              axios.post('/api/crm/adPricesystem/save',qs.stringify(this.inputForm)).then((response)=> {
          ...
          if(this.selectChange){
            ...
            form.validate((valid) => {
              if (valid) {
                this.inputForm.newDepotIds = this.selects;
                axios.post('/api/crm/adPricesystem/save',qs.stringify(this.inputForm, {allowDots:true})).then((response)=>{
                  this.$message(response.data.message);
                  this.selectChange = false;
                  this.getDepots();
                })
        ...
        if(!this.isCreate){
          axios.get('/api/crm/adPricesystem/findOne',{params: {id:this.$route.query.id}}).then((response)=>{
