<template>
  <div>
    <head-tab :active="$t('bankForm.bankForm') "></head-tab>
    <div>
    ...
    export default{
      ...
      methods:{
        formSubmit(){
          ...
          form.validate((valid) => {
            if (valid) {
              axios.post('/api/crm/bank/save',qs.stringify(this.inputForm)).then((response)=> {
        ...
        if(!this.isCreate){
          axios.get('/api/crm/bank/findOne',{params: {id:this.$route.query.id}}).then((response)=>{
