    ...
    export default{
      ...
      methods:{
        formSubmit(){
          ...
          form.validate((valid) => {
            if (valid) {
              axios.post('/api/sys/dictMap/save', qs.stringify(this.inputForm)).then((response)=> {
        ...
        axios.get('/api/sys/dictMap/getFormProperty').then((response)=>{
          this.formProperty=response.data;
        });
        if(!this.isCreate){
          axios.get('/api/sys/dictMap/findOne',{params: {id:this.$route.query.id}}).then((response)=>{
