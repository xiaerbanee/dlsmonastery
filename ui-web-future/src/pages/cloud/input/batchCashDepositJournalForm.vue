<template>
  <div>
    <head-tab active="batchCashDepositJournalForm"></head-tab>
    <div>
...
<script>
  ...
  export default {
    data() {
      return {
        ...
        settings: {
          ...
          stretchH: 'all',
          height: 650,
          colHeaders: ["对方科目","对方科目全称","借方金额","贷方金额","摘要","员工","部门","其他类","费用类"],
          columns:[
    ...
    mounted() {
      axios.get('/api/global/cloud/input/batchCashDepositJournal/form').then((response)=>{
    ...
    methods: {
      formSubmit(){
        ...
        axios.post('/api/global/cloud/input/batchCashDepositJournal/save', qs.stringify(this.formData,{allowDots:true})).then((response)=> {
