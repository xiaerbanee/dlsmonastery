<template>
  <div>
    ...
    <div>
      ...
      <search-dialog :show="formVisible" @hide="formVisible=false" :title="$t('shopBuildList.filter')" v-model="formVisible" size="medium" class="search-form" z-index="1500" ref="searchDialog">
      ...
      <el-table :data="page.content" :height="pageHeight" style="margin-top:5px;" v-loading="pageLoading" :element-loading-text="$t('shopBuildList.loading')" @sort-change="sortChange" @selection-change="handleSelectionChange" stripe border>
        ...
        <el-table-column fixed="right" :label="$t('shopBuildList.operation')" width="140">
          <template scope="scope">
            ...
            <div class="action" v-if="((scope.row.isEditable&&!scope.row.locked)||scope.row.isAuditable)&&scope.row.processStatus.indexOf('通过')<0" v-permit="'crm:shopBuild:edit'"><el-button size="small" @click.native="itemAction(scope.row.id,'edit')">{{$t('shopBuildList.edit')}}</el-button></div>
            <div class="action" v-if="scope.row.isEditable&&!scope.row.locked" v-permit="'crm:shopBuild:delete'"><el-button size="small" @click.native="itemAction(scope.row.id,'delete')">{{$t('shopBuildList.delete')}}</el-button></div>
          </template>
        </el-table-column>
      </el-table>
