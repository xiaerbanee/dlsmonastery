<template>
  <div>
    ...
    <div>
      <el-row>
        <el-button type="primary" @click="itemAdd" icon="plus" v-permit="'sys:dictEnum:edit'">{{$t('recruitEnumList.add')}}</el-button>
        <el-button type="primary"@click="formVisible = true" icon="search" v-permit="'sys:dictEnum:view'">{{$t('recruitEnumList.filter')}}</el-button>
      ...
      <search-dialog @enter="search()" :show="formVisible" @hide="formVisible=false" :title="$t('recruitEnumList.filter')" v-model="formVisible" size="tiny" class="search-form" z-index="1500" ref="searchDialog">
        <el-form :model="formData" :label-width="formLabelWidth">
          ...
          <el-form-item :label="$t('recruitEnumList.value')">
            <el-input v-model="formData.value" auto-complete="off" :placeholder="$t('recruitEnumList.likeSearch')"></el-input>
          </el-form-item>
        </el-form>
      ...
      <el-table :data="page.content" :height="pageHeight" style="margin-top:5px;" v-loading="pageLoading" :element-loading-text="$t('recruitEnumList.loading')" @sort-change="sortChange" stripe border>
        <el-table-column fixed prop="id" :label="$t('recruitEnumList.id')" sortable width="150"></el-table-column>
        <el-table-column prop="category" :label="$t('recruitEnumList.category')"></el-table-column>
        <el-table-column prop="value" :label="$t('recruitEnumList.value')"></el-table-column>
        ...
        <el-table-column prop="locked" :label="$t('recruitEnumList.locked')" width="120">
          <template scope="scope">
            <el-tag :type="scope.row.locked ? 'primary' : 'danger'">{{scope.row.locked | bool2str}}</el-tag>
          </template>
        </el-table-column>
        <el-table-column fixed="right" :label="$t('recruitEnumList.operation')" width="140">
          <template scope="scope">
            <el-button size="small" @click.native="itemAction(scope.row.id,'edit')" v-permit="'sys:dictEnum:edit'">{{$t('recruitEnumList.edit')}}</el-button>
            <el-button size="small" @click.native="itemAction(scope.row.id,'delete')" v-permit="'sys:dictEnum:delete'">{{$t('recruitEnumList.delete')}}</el-button>
          </template>
        </el-table-column>
      </el-table>
  ...
  export default{
    ...
    methods: {
      ...
      pageRequest() {
        ...
        if(action=="edit") {
          this.$router.push({ name: 'recruitEnumForm', query: { id: id }})
