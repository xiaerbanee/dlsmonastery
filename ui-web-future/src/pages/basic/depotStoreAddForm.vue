<template>
  <div>
    <head-tab active="depotStoreAddForm"></head-tab>
    <div>
  ...
  export default {
    ...
    methods:{
      ...
      formSubmit(){
        ...
        let depotForm = this.$refs["depotForm"];
        form.validate((valid) => {
          depotForm.validate((depotValid) => {
            if (valid&&depotValid) {
              let submitData=util.deleteExtra(this.inputForm);
              submitData.depotForm=this.depotDto;
              axios.post('/api/ws/future/basic/depotStore/saveStore', qs.stringify(submitData, {allowDots:true})).then((response)=> {
        ...
        axios.get('/api/ws/future/basic/depotStore/getAddForm').then((response)=>{
