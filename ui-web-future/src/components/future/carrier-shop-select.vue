<template>
  <div>
    <el-select v-model="innerId"  filterable remote :multiple="multiple" :disabled="disabled" :placeholder="$t('su_district.inputKey')" :remote-method="remoteSelect" :loading="remoteLoading"  :clearable=true @change="handleChange">
      <el-option v-for="item in itemList"  :key="item.id" :label="(this.util.isNotBlank(item.code)?item.code+'_':'')+item.name" :value="item.id"></el-option>
    </el-select>
  </div>
</template>
<script>
  export default {
    props: ['value', 'multiple', 'disabled', 'type'],
    data() {
      return {
        innerId: null,
        itemList: [],
      ...
      remoteSelect(query) {
        ...
        axios.get('/api/ws/future/api/carrierShop/search', {params: {name: query,type:this.type}}).then((response) => {
        ...
        if (val) {
          ...
          this.remoteLoading = true;
          axios.get('/api/ws/future/api/carrierShop/findOne?id=' + ids).then((response) => {
