<template>
  <div>
    <el-select v-model="innerId"  filterable :multiple="isMultiple" remote :placeholder="$t('su_district.inputKey')" :remote-method="remoteSelect" :loading="remoteLoading"  :clearable=true @change="handleChange">
  ...
  export default {
    ...
    data() {
      ...
      remoteSelect(query) {
        ...
        this.innerId=val;
        this.remoteLoading = true;
        axios.get('/api/future/basic/depot/searchById?id=' + this.innerId).then((response)=>{
          this.itemList=response.data;
          this.remoteLoading = false;
        })
      },
