<template>
  <div>
    <el-select  ref="select" v-model="innerId"  filterable remote :multiple="multiple" :disabled="disabled" isIme="isIme" :placeholder="$t('su_district.inputKey')" :remote-method="remoteSelect" :loading="remoteLoading"  :clearable=true @change="handleChange">
...
<script>
  export default {
    props: ['value','multiple','disabled',"isIme"],
    data() {
      return {
        innerId:this.value,
        itemList : [],
      ...
      remoteSelect(query) {
        ...
        axios.get("/api/ws/future/basic/product/searchFullText", {params:{key:query,isIme:isIme}}).then((response)=>{
