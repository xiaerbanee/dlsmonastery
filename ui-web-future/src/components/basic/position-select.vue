<template>
  <div>
    <el-select v-model="innerId"  filterable remote :multiple="isMultiple" :disabled="isDisabled" :remote-method="remoteSelect" :loading="remoteLoading"  :clearable=true @change="handleChange">
  ...
  export default {
    ...
    data() {
      ...
      remoteSelect(query) {
        ...
        this.innerId=val;
        this.remoteLoading = true;
        axios.get('/api/basic/hr/position/findById?id=' + this.innerId).then((response)=>{
          this.itemList=response.data;
          this.remoteLoading = false;
        })
      },
